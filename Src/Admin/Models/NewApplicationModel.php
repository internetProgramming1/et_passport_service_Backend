...
class NewApplicationModel
{
    ...
    {
        $stmt = $this->pdo->prepare("SELECT * FROM urgent_new_application");
        $stmt->execute();
    ...
    {
        $stmt = $this->pdo->prepare("SELECT * FROM new_application");
        $stmt->execute();
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
    public function getApplicationNo($appNo)
    {
        ...
        $stmt = $this->pdo->prepare("SELECT * FROM new_application WHERE application_no = ?");
        $stmt->execute([$appNo]);
        ...
        $stmt = $this->pdo->prepare("SELECT * FROM urgent_new_application WHERE applicationNumber = ?");
        $stmt->execute([$appNo]);
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
