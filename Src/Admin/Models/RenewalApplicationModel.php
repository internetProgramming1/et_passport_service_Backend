...
class RenewalApplicationModel
{
    ...
    public function getApplicationNo($appNo)
    {
        ...
        $stmt = $this->pdo->prepare("SELECT * FROM urgent_renewal_application WHERE application_no = ?");
        $stmt->execute([$appNo]);
        ...
        $stmt = $this->pdo->prepare("SELECT * FROM renewal_application WHERE application_no = ?");
        $stmt->execute([$appNo]);
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
