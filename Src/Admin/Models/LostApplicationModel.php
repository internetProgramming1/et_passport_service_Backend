...
class LostApplicationModel
{
    ...
    public function getApplicationNo($appNo)
    {
        ...
        $stmt = $this->pdo->prepare("SELECT * FROM lost_application WHERE application_no = ?");
        $stmt->execute([$appNo]);
        ...
        $stmt = $this->pdo->prepare("SELECT * FROM urgent_lost_application WHERE applicationNumber = ?");
        $stmt->execute([$appNo]);
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
    ...
    public function updateStatus($id, $status)
    {
        try {
            ...
            UPDATE urgent_lost_application 
